...
add_args_from_env_v10(){
    args_bool "$INPUT_PRERELEASE" "--as-prerelease" ""
...
main(){
    ...
    case ${version} in
        ...
        "8")
            add_args_from_env_v8;;
        *)
            echo "Semantic release version not supported ($version)" >&2 
            exit 2
    esac
